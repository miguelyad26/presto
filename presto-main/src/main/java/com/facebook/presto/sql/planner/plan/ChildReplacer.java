        ...
        extends PlanVisitor<List<PlanNode>, PlanNode>
{
    ...
    public PlanNode visitJoin(JoinNode node, List<PlanNode> newChildren)
    {
        ...
        return new JoinNode(node.getId(), node.getType(), node.getMethod(), newChildren.get(0), newChildren.get(1), node.getCriteria(), node.getFilter(), node.getLeftHashSymbol(), node.getRightHashSymbol());
    }
